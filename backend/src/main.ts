...
async function bootstrap() {
  ...
  app.enableCors({
    origin: (origin, callback) => {
      ...
      logger.warn(
        `CORS: Blocked request from disallowed origin: ${safeOrigin}`,
      );
  ...
  logger.log(
    `🔒 CORS configured — allowed origins: ${effectiveOrigins.join(", ") || "(none)"}`,
  );
  ...
  if (
    nodeEnv === "development" ||
    configService.get<boolean>("ENABLE_SWAGGER_DOCS", true)
  ) {
    const config = new DocumentBuilder()
      .setTitle("Nivesh API")
      .setDescription(
        "AI-Native Financial Reasoning Platform API Documentation",
      )
      .setVersion("1.0")
  ...
  logger.log(
    `🚀 Nivesh Backend is running on: http://localhost:${port}/${apiPrefix}`,
  );
