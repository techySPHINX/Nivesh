...
export class GetSimulationHistoryHandler implements IQueryHandler<GetSimulationHistoryQuery> {
  constructor(
    @Inject(SIMULATION_REPOSITORY)
    private readonly simulationRepository: ISimulationRepository,
  ) {}

  async execute(query: GetSimulationHistoryQuery): Promise<{
    ...
    const { simulations, total } =
      await this.simulationRepository.findByUserIdPaginated(
        userId,
        skip,
        limit,
      );
