...
import { ResumeGoalCommand } from '../commands/resume-goal.command';
...
@CommandHandler(ResumeGoalCommand)
export class ResumeGoalHandler implements ICommandHandler<ResumeGoalCommand, GoalResponseDto> {
  constructor(
    @Inject(GOAL_REPOSITORY) private readonly goalRepository: IGoalRepository,
    private readonly eventBus: EventBus,
  ) { }

  async execute(command: ResumeGoalCommand): Promise<GoalResponseDto> {
    ...
    if (goal.userId !== userId) {
      throw new ForbiddenException('You do not have permission to resume this goal');
    }
    ...
    // Publish event
    const event = new GoalUpdatedEvent(goalId, userId, { status: 'ACTIVE' });
