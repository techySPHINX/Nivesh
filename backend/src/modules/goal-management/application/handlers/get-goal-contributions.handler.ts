...
export class GetGoalContributionsHandler implements IQueryHandler<
  GetGoalContributionsQuery,
  ...
  async execute(
    ...
    if (goal.userId !== userId) {
      throw new ForbiddenException(
        "You do not have permission to view contributions for this goal",
      );
    }

    // Fetch contributions
    const contributions =
      await this.contributionRepository.findByGoalId(goalId);

    return contributions.map((contribution) =>
      this.toResponseDto(contribution),
    );
  }
