...
describe("Money Value Object", () => {
  describe("Creation", () => {
    ...
    it("should throw error for negative amount", () => {
      ...
      expect(() => new Money(-100, Currency.INR)).toThrow(
        "Money amount cannot be negative",
      );
    });

    it("should throw error for amount exceeding maximum", () => {
      expect(() => new Money(1000000001, Currency.INR)).toThrow(
        DomainException,
      );
      expect(() => new Money(1000000001, Currency.INR)).toThrow(
        "Money amount cannot exceed 1 billion",
      );
    });

    it("should throw error for invalid currency", () => {
      expect(() => new Money(100, "INVALID" as Currency)).toThrow(
        DomainException,
      );
      expect(() => new Money(100, "INVALID" as Currency)).toThrow(
        "Invalid currency",
  ...
  describe("Arithmetic Operations", () => {
    describe("Addition", () => {
      ...
      it("should handle decimal additions correctly", () => {
        const money1 = new Money(10.5, Currency.INR);
