...
describe("LLM Integration E2E Tests", () => {
  ...
  describe("POST /api/v1/ai/structured", () => {
    it("should return structured output matching schema", async () => {
      ...
      expect(["BUY", "HOLD", "SELL"]).toContain(
        response.body.data.recommendation,
      );
  ...
  describe("POST /api/v1/ai/function-call", () => {
    ...
    it("should execute loan affordability function", async () => {
      const response = await request(app.getHttpServer())
        ...
        .send({
          prompt:
            "Can I afford ₹50 lakh home loan with ₹1.5 lakh monthly income?",
          userId: testUserId,
          enableFunctions: true,
        })
        .expect(201);
    ...
    it("should execute SIP returns calculation", async () => {
      const response = await request(app.getHttpServer())
        ...
        .send({
          prompt:
            "What will I get if I invest ₹10,000 monthly for 20 years at 12% return?",
          userId: testUserId,
          enableFunctions: true,
        })
        .expect(201);
    ...
    it("POST /api/v1/ai/prompts - should create prompt", async () => {
      const response = await request(app.getHttpServer())
        ...
        .send({
          ...
          promptText:
            "You are a helpful financial advisor. Answer in simple terms.",
          version: "1.0.0",
          status: "DRAFT",
          temperature: 0.8,
          topP: 0.95,
  ...
  describe("A/B Testing", () => {
    ...
    afterAll(async () => {
      await prisma.promptABTest.deleteMany({
        where: { testName: "tone-test" },
      });
      await prisma.promptRegistry.deleteMany({
        where: { promptName: "ab-test-prompt" },
      });
    });
  });
