...
export class Neo4jService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    try {
      const uri =
        this.configService.get<string>("database.neo4j.uri") ||
        "bolt://localhost:7687";
      const username =
        this.configService.get<string>("database.neo4j.username") || "neo4j";
      const password =
        this.configService.get<string>("database.neo4j.password") || "password";
  ...
  getSession(database?: string): Session {
    const db =
      database || this.configService.get<string>("database.neo4j.database");
