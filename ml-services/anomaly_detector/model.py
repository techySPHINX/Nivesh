...
class AnomalyDetector:
    ...
    def engineer_features(
        ...
        if 'user_id' in df.columns and user_profiles is None:
            ...
            df['amount_zscore'] = (
                df['amount'] - df['user_mean']) / (df['user_std'] + 1e-5)
        elif user_profiles:
        ...
        if 'user_id' in df.columns and 'date' in df.columns:
            daily_counts = df.groupby(['user_id', df['date'].dt.date]).size(
        ...
        logger.info(
            f"Engineered {len(df.columns)} features from {len(df)} transactions")
    ...
    def prepare_data(
        ...
        logger.info(
            f"Prepared {len(X)} samples with {len(numeric_cols)} features")
    ...
    def train(
        ...
        logger.info(
            f"Training Isolation Forest with contamination={self.contamination}")
        ...
        if y is not None:
            ...
            metrics['precision'] = float(
                np.sum((predictions == -1) & (y == -1)) / max(1, np.sum(predictions == -1)))
            metrics['recall'] = float(
                np.sum((predictions == -1) & (y == -1)) / max(1, np.sum(y == -1)))
        ...
        logger.info(
            f"Training complete. Detected {metrics['anomalies_detected']} anomalies ({metrics['anomaly_rate']:.2%})")
    ...
    def predict(
        ...
        Raises:
            ValueError: If model not trained or transaction invalid
        ...
        try:
            ...
            if amount > 1e10:  # Sanity check
                logger.warning(f"Extremely large transaction amount: {amount}")
