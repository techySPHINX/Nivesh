...
class MLConfig(BaseSettings):
    ...
    feature_ttl_seconds: int = Field(default=3600, env="FEATURE_TTL_SECONDS")  # 1 hour
    ...
    # Model Storage
    model_storage_backend: str = Field(default="local", env="MODEL_STORAGE_BACKEND")  # local, s3, gcs
    ...
    intent_model_name: str = Field(default="distilbert-base-uncased", env="INTENT_MODEL_NAME")
    ...
    # Spending Prediction
    spending_forecast_horizon: int = Field(default=365, env="SPENDING_FORECAST_HORIZON")  # days
    
    # Anomaly Detection
    anomaly_contamination: float = Field(default=0.01, env="ANOMALY_CONTAMINATION")  # 1%
    anomaly_threshold: float = Field(default=-0.5, env="ANOMALY_THRESHOLD")
    
    # Credit Risk
    credit_risk_threshold: float = Field(default=0.5, env="CREDIT_RISK_THRESHOLD")
    ...
    drift_alert_threshold: float = Field(default=0.3, env="DRIFT_ALERT_THRESHOLD")
